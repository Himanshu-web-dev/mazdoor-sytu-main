...
function computeWorkerDistance(worker, currentArea, currentCity = '') {
  // If worker is located in Jhansi:
  if (worker.location === 'Jhansi') {
    return {
      distanceKm: worker.distanceKm || 1.8,
      eta: worker.eta || '15–20 min',
      isImmediate: true,
    }
  }
...
export default function Workers({ onNavigate }) {
  ...
  const { immediateWorkers, scheduledWorkers } = useMemo(() => {
    ...
    // 2. Scheduled: ALL Meerut/Jhansi workers + regional craftsmen
    const scheduled = filtered.filter((w) => w.location === 'Meerut' || w.location === 'Jhansi' || w.computedDistance > 10.0)
  ...
  const filteredLocalities = useMemo(() => {
    const list = customerCity === 'Jhansi' ? JHANSI_LOCALITIES : MEERUT_LOCALITIES
    if (!areaSearch) return list
    const q = areaSearch.toLowerCase().trim()
    return list.filter((l) => l.name.toLowerCase().includes(q))
  }, [areaSearch, customerCity])
