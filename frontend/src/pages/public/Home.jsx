...
const capabilities = [
  {
    titleKey: 'cap_verified_profiles',
    descKey: 'cap_verified_profiles_desc',
  ...
  {
    titleKey: 'cap_nearby_discovery',
    descKey: 'cap_nearby_discovery_desc',
  ...
  {
    titleKey: 'cap_realtime_avail',
    descKey: 'cap_realtime_avail_desc',
  ...
  {
    titleKey: 'cap_scheduled',
    descKey: 'cap_scheduled_desc',
  ...
  {
    titleKey: 'cap_live_tracking',
    descKey: 'cap_live_tracking_desc',
  ...
  {
    titleKey: 'cap_secure_payments',
    descKey: 'cap_secure_payments_desc',
  ...
  {
    titleKey: 'cap_digital_invoices',
    descKey: 'cap_digital_invoices_desc',
  ...
  {
    titleKey: 'cap_platform_comm',
    descKey: 'cap_platform_comm_desc',
  ...
  {
    titleKey: 'cap_dedicated_support',
    descKey: 'cap_dedicated_support_desc',
...
const trustPillars = [
  {
    titleKey: 'trust_identity',
    descKey: 'trust_identity_desc',
  ...
  {
    titleKey: 'trust_reviews',
    descKey: 'trust_reviews_desc',
  ...
  {
    titleKey: 'trust_pricing',
    descKey: 'trust_pricing_desc',
  ...
  {
    titleKey: 'trust_safety',
    descKey: 'trust_safety_desc',
...
export default function Home({ onNavigate }) {
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => {
      if (locFieldRef.current && !locFieldRef.current.contains(e.target)) {
  ...
  const handleSelectSuggestion = (item) => {
    ...
    setLocationSource('manual')
    saveLocation(full, item.city, item.area, item.lat ? { latitude: item.lat, longitude: item.lon } : null)
    setShowLocSuggestions(false)
  }
  ...
  useEffect(() => {
    ...
    const handleLocUpdate = (e) => {
      if (e.detail?.location) {
        ...
        setLocationSource(e.detail.coords ? 'gps' : 'auto')
        setIsLocating(false)
      }
    }
  ...
  return (
    <div className="home-container">
          ...
          <div className="hero-left">
            ...
            <div className="hero-search-card">
              <form className="hero-search-form" onSubmit={handleSearchSubmit}>
                ...
                <div className="search-field-group" ref={locFieldRef} style={{ position: 'relative' }}>
                  <button
                    type="button"
                    className="btn-open-loc-modal-icon"
                    onClick={() => setIsLocModalOpen(true)}
                    title="Click to pick your city or locality"
                    aria-label="Pick location from list"
                  ...
                  <div className="search-field-inner">
                    ...
                    <input
                      ...
                      translate="no"
                      autoComplete="off"
                      placeholder={isLocating ? 'Detecting your location...' : t('hero_location_placeholder', 'Enter city or locality (e.g. Meerut, Noida)')}
                      value={locationQuery}
                      onFocus={() => {
                  ...
                  {showLocSuggestions && locSuggestions.length > 0 && (
                    <div className="hero-loc-suggestions notranslate" translate="no">
                      <div className="hero-loc-suggestions-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', fontSize: '11px', fontWeight: '700', color: '#64748b' }}>
                        <span>{locationQuery ? 'MATCHING LOCATIONS' : 'POPULAR CITIES & AREAS'}</span>
                        <button
                          type="button"
                          className="btn-suggestion-modal-link"
                          style={{ background: 'none', border: 'none', color: '#008744', fontWeight: '700', cursor: 'pointer', padding: 0, fontSize: '11px' }}
                          onClick={(e) => {
                            e.stopPropagation()
                            setShowLocSuggestions(false)
                            setIsLocModalOpen(true)
                      ...
                      {locSuggestions.map((item, idx) => (
                        <div
                          key={`${item.label}-${idx}`}
                          className="hero-loc-suggestion-item"
                          onClick={() => handleSelectSuggestion(item)}
                        >
                          <span className="sugg-pin">📍</span>
                          <div className="sugg-text">
                            <strong>{item.area ? item.area : item.city}</strong>
                            <small>{item.area ? item.city : 'City Directory'}</small>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className="why-us-container">
          ...
          <div className="why-us-metrics-ribbon" ref={metricsRibbonRef}>
            <div className="metric-ribbon-item item-highlight-green">
              <div className="metric-badge-chip chip-green">
                ...
                <span>{t('metric_verified_deliveries', 'Verified Deliveries')}</span>
              </div>
              ...
              <span className="ribbon-label">{t('metric_bookings_completed', 'Bookings Completed')}</span>
            </div>
            ...
            <div className="metric-ribbon-item item-highlight-amber">
              <div className="metric-badge-chip chip-amber">
                ...
                <span>{t('metric_rapid_dispatch', 'Rapid Dispatch')}</span>
              </div>
              <strong className="ribbon-num num-glow-dark">
                15–30 min
              </strong>
              <span className="ribbon-label">{t('metric_arrival_time', 'Average Arrival Time')}</span>
            </div>
            ...
            <div className="metric-ribbon-item item-highlight-blue">
              <div className="metric-badge-chip chip-blue">
                ...
                <span>{t('metric_aadhaar_verified', 'Aadhaar Verified')}</span>
              </div>
              <strong className="ribbon-num num-glow-blue">
                <AnimatedMetricCounter target={5000} duration={1900} isVisible={metricsVisible} suffix="+" />
              </strong>
              <span className="ribbon-label">{t('metric_verified_tradesmen', 'Verified Tradesmen')}</span>
            </div>
            ...
            <div className="metric-ribbon-item item-highlight-gold">
              <div className="metric-badge-chip chip-gold">
                <span className="chip-icon">★</span>
                <span>{t('metric_customer_trust', 'Customer Trust')}</span>
              </div>
              ...
              <span className="ribbon-label">{t('metric_customer_satisfaction', 'Customer Satisfaction')}</span>
            </div>
          </div>
          ...
          <div className="why-us-cards-grid">
            ...
            <article className="why-us-card">
              ...
              <h3 className="why-us-card-title">{t('pillar_vetting_title', 'Government ID & Police Vetting')}</h3>
              <p className="why-us-card-desc">
                {t('pillar_vetting_desc', 'Every tradesperson undergoes biometric Aadhaar authentication, trade certification review, and background screening before accepting platform bookings.')}
              </p>
              <div className="why-us-card-spec">
                <span className="spec-bullet" />
                <span>{t('pillar_vetting_spec', '100% ID Verified')}</span>
              </div>
            </article>
            ...
            <article className="why-us-card">
              ...
              <h3 className="why-us-card-title">{t('pillar_rates_title', 'Standardized Upfront Rates')}</h3>
              <p className="why-us-card-desc">
                {t('pillar_rates_desc', 'Clear, published rate schedules with itemized service estimates. No unexpected surcharges or arbitrary bargaining — pay securely upon completion.')}
              </p>
              <div className="why-us-card-spec">
                <span className="spec-bullet" />
                <span>{t('pillar_rates_spec', 'Zero Hidden Fees')}</span>
              </div>
            </article>
            ...
            <article className="why-us-card">
              ...
              <h3 className="why-us-card-title">{t('pillar_dispatch_title', 'Rapid Hyperlocal Dispatch')}</h3>
              <p className="why-us-card-desc">
                {t('pillar_dispatch_desc', 'Intelligent routing matches your requirement with top-rated professionals active within 2 to 5 km of your address for timely doorstep arrival.')}
              </p>
              <div className="why-us-card-spec">
                <span className="spec-bullet" />
                <span>{t('pillar_dispatch_spec', '15–30 Min Arrival')}</span>
              </div>
            </article>
            ...
            <article className="why-us-card">
              ...
              <h3 className="why-us-card-title">{t('pillar_warranty_title', 'Workmanship Warranty')}</h3>
              <p className="why-us-card-desc">
                {t('pillar_warranty_desc', 'Platform mediation, dedicated customer assistance, and a complimentary 7-day rework guarantee on eligible services for complete peace of mind.')}
              </p>
              <div className="why-us-card-spec">
                <span className="spec-bullet" />
                <span>{t('pillar_warranty_spec', '7-Day Rework Guarantee')}</span>
              </div>
            </article>
          </div>
      ...
      {/* 
          5. PLATFORM ECOSYSTEM: HOUSEHOLDS, ENTERPRISES, ARTISANS
           */}
        ...
        <div className="capabilities-grid">
          {capabilities.map((cap, i) => (
            <div className="capability-card" key={i}>
              ...
              <h3>{t(cap.titleKey, cap.title)}</h3>
              <p>{t(cap.descKey, cap.description)}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="trust-pillars-grid">
          {trustPillars.map((pillar, i) => (
            <div className="trust-pillar-card" key={i}>
              ...
              <h3>{t(pillar.titleKey, pillar.title)}</h3>
              <p>{t(pillar.descKey, pillar.description)}</p>
            </div>
          ))}
        </div>
      </section>
