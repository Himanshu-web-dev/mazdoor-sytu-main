...
function Navbar({ onNavigate, session, onLogout }) {
  ...
  const [userLoc, setUserLoc] = useState(() => {
    ...
    if (saved.area && saved.city && saved.area.toLowerCase() !== saved.city.toLowerCase()) {
      return `${saved.area}, ${saved.city}`
    }
    return saved.city || saved.location || ''
  })
    ...
    const handleLocUpdate = (e) => {
      if (e.detail) {
        ...
        if (area && city && area.toLowerCase() !== city.toLowerCase()) {
          setUserLoc(`${area}, ${city}`)
        } else {
          setUserLoc(city || location || '')
        }
      }
    }
    ...
    <>
      <header className="site-header">
        <div className="site-nav">
          <div className="brand-group">
            <button className="brand" type="button" onClick={() => handleNavClick('/')} title="Mazdoor Sytu Home">
              <img className="brand-logo" src="/official-logo.png" alt="Mazdoor Sytu logo" />
              <div className="brand-title-wrap notranslate" translate="no">
                <span className="brand-name notranslate" translate="no">Mazdoor Sytu</span>
              </div>
            </button>
            <button
              type="button"
              className="brand-loc-badge notranslate"
              translate="no"
              onClick={() => setIsLocModalOpen(true)}
              title="Click to change your delivery city/area or auto-detect via GPS"
              aria-label="Change current location"
            >
              <span className="loc-dot-pulse" />
              <span className="brand-loc-city">{userLoc || 'Set Location'}</span>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ width: 10, height: 10, marginLeft: 2, flexShrink: 0 }}>
                <path d="m6 9 6 6 6-6" />
              </svg>
            </button>
          </div>
        ...
        <div className="nav-auth-wrap">
          ...
          <div className="nav-auth">
            ...
            <div className="nav-lang-picker notranslate" translate="no" ref={langPickerRef}>
              <button
                type="button"
                className={`nav-lang-btn notranslate ${isLangOpen ? 'active' : ''}`}
                translate="no"
                onClick={() => {
              ...
              >
                ...
                <span className="lang-current-label notranslate" translate="no">{currentLangObj.native}</span>
                <svg
              ...
              {isLangOpen && (
                <div className="nav-lang-dropdown notranslate" translate="no" role="listbox" aria-label="Available languages">
                  <div className="lang-dropdown-header notranslate" translate="no">
                    <span>Choose Language / भाषा</span>
                    <span className="lang-count-badge notranslate" translate="no">{LANGUAGES.length} Languages</span>
                  </div>

                  {/* Search box for 16 languages */}
                  <div className="lang-search-wrap notranslate" translate="no">
                    <svg
      ...
      >
        ...
        <div className="mobile-drawer-footer">
          ...
          <div className="mobile-lang-section notranslate" translate="no">
            <div className="mobile-lang-title notranslate" translate="no">
              ...
              <span className="lang-count-badge notranslate" translate="no">{LANGUAGES.length}</span>
            </div>
            <div className="mobile-lang-chips notranslate" translate="no">
              {LANGUAGES.map((lang) => (
                <button
                  key={lang.code}
                  type="button"
                  className={`mobile-lang-chip notranslate ${language === lang.code ? 'selected' : ''}`}
                  translate="no"
                  onClick={() => handleLanguageSelect(lang.code)}
                >
                  {lang.native}
      ...
      {/* Global Interactive Location Selector Modal */}
      <LocationModal
