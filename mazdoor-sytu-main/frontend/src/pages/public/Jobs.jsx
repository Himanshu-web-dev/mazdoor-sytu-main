...
export default function Jobs({ onNavigate, session, workerData }) {
  ...
  const handleQuickDemoWorkerLogin = (job) => {
    const demoUser = {
      name: 'Rahul Kumar',
      email: 'rahul.kumar@example.com',
      role: 'worker',
      phone: '+91 98765 43210',
      trade: job?.category || 'Electrician',
      authenticated: true,
    }
    try {
      localStorage.setItem('mazdoor_sytu_session_v1', JSON.stringify(demoUser))
      localStorage.setItem('mazdoor-Setu-session', JSON.stringify(demoUser))
    } catch {}
      ...
      */}
      <section className="jobs-live-ribbon" aria-label="Customer Locality & Stats">
        ...
        <div className="jobs-ribbon-metrics">
          ...
          <div className="ribbon-metric-pill highlight-green">
            <span className="pill-num">0% Commission</span>
            <span className="pill-lbl">Direct Contractor Pay</span>
          </div>
        </div>
      </section>
            ...
            <div className="modal-body-scroll" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div style={{ borderTop: '1px solid #e2e8f0', marginTop: '12px', paddingTop: '14px' }}>
                  <button
                    type="button"
                    onClick={() => handleQuickDemoWorkerLogin(authRequiredJob)}
                    style={{
                      width: '100%',
                      background: '#f8fafc',
                      border: '1.5px dashed #008744',
                      borderRadius: '10px',
                      padding: '11px',
                      fontSize: '13px',
                      fontWeight: 700,
                  ...
                  >
                    <span>⚡</span>
                    <span>Quick Demo Worker Login (Test as Rahul Kumar)</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="modal-body-scroll" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <button
                  ...
                  className="btn-card-apply"
                  style={{ width: '100%', justifyContent: 'center', padding: '12px' }}
                  onClick={() => handleQuickDemoWorkerLogin(roleMismatchInfo.job)}
                >
                  Switch to Worker Account (Rahul Kumar)
                </button>
                <button
                  type="button"
                  className="btn-card-view-scope"
